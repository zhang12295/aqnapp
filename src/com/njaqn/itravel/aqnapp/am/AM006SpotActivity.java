...
public class AM006SpotActivity extends Activity {
	...
	private void initData() {
		...
		String r = null;
		try {
			r = url.postRequestForSql("select intro,TitleImage from J_Spot where ID = "+id, AQNAppConst.DB_ONE_MANY);
			if(r.equals("Err")){
				Toast.makeText(AM006SpotActivity.this, "暂无网络", Toast.LENGTH_SHORT).show();
			}else{
			...
			String t = url.postRequestForSql("select TitleImage from J_JingDian where SpotID="+id,AQNAppConst.DB_MANY_MANY );
			if(t.equals("Err")){
		...
		mImageView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(mAtlas.VISIBLE==View.GONE){
					Toast.makeText(AM006SpotActivity.this, "该景区暂无其他景点", Toast.LENGTH_SHORT).show();
				}else{
					Intent intent = new Intent(AM006SpotActivity.this,AM006SpotIntroActivity.class);
	...
	class imageButtonPlayListener implements OnClickListener {
		public void onClick(View v) {
			...
			case R.id.iv_gotospot:
				Intent i = new Intent(AM006SpotActivity.this,AM001HomePageActivity.class);
				i.putExtra("id", id);
				startActivity(i);
				break;
